# Copied and adapted from https://github.com/elixir-lang/plug/blob/master/test/plug/csrf_protection_test.exs
defmodule PlugRailsCSRFProtectionTest do
  use ExUnit.Case, async: true
  use Plug.Test

  alias PlugRailsCSRFProtection
  alias PlugRailsCSRFProtection.InvalidCSRFTokenError
  alias PlugRailsCSRFProtection.InvalidCrossOriginRequestError
  ...
  defp call_csrf_with_session(conn, csrf_plug_opts) do
    ...
    |> fetch_session
    |> put_session("key", "val")
    |> PlugRailsCSRFProtection.call(PlugRailsCSRFProtection.init(csrf_plug_opts))
  ...
  defp handle_token(conn) do
    case conn.params["token"] do
      "get" ->
        send_resp(conn, 200, PlugRailsCSRFProtection.get_csrf_token())
      "delete" ->
        PlugRailsCSRFProtection.delete_csrf_token()
        send_resp(conn, 200, "")
      _ ->
        send_resp(conn, 200, "")
    end
  ...
  test "token is stored in process dictionary" do
    assert PlugRailsCSRFProtection.get_csrf_token() ==
           PlugRailsCSRFProtection.get_csrf_token()

    t1 = PlugRailsCSRFProtection.get_csrf_token
    PlugRailsCSRFProtection.delete_csrf_token
    assert t1 != PlugRailsCSRFProtection.get_csrf_token
  end
  ...
  test "generated tokens are always masked" do
    ...
    assert byte_size(conn1.resp_body) == 88

    conn2 = call(conn(:get, "/?token=get"))
    assert byte_size(conn2.resp_body) == 88
