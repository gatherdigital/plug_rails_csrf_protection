defmodule PlugRailsCSRFProtection do
  @moduledoc """
  Plug to protect from cross-site request forgery in a Rails compatible way.
  Copied and adapted from https://github.com/elixir-lang/plug/blob/master/lib/plug/csrf_protection.ex
  ...
  defp valid_csrf_token?(<<csrf_token::@token_size-binary>>, <<encoded_token::@double_encoded_token_size-binary>>) do
    case Base.decode64(encoded_token) do
      {:ok, masked_token} -> valid_csrf_token?(csrf_token, masked_token)
      _ -> false
    end
  end

  defp valid_csrf_token?(<<csrf_token::@token_size-binary>>,
                         <<mask::@token_size-binary, user_token::@token_size-binary>>) do
  ...
  defp mask(token) do
    mask = generate_token()
    Base.encode64(mask <> Plug.Crypto.mask(token, mask))
  end
